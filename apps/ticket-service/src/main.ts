...
import { createEmailIngestRoute } from "./routes/email-ingest.js";

const config = getServiceConfig("ticket-service");
...
app.use("/tickets", ticketRoutes(db, config, logger));
app.use("/tickets", messageRoutes(db, config, logger));
app.use("/tickets", attachmentRoutes(db, config, logger));
app.use("/tickets", createEmailIngestRoute(db, config, logger));
app.use("/buckets", bucketRoutes(db, config, logger));
